...
namespace http {
    namespace ws {
        ...
        ssize_t Response::send(const std::string& msg) {
        ...
        Result Response::read(std::string& msg) {
            if (!readFrame(byte_data)) {
                return Result::Fail;
            }

            msg = frame.text_payload;
            return frame.opcode == Opcode::Text ? Result::Text : Result::Binary;
        }

        bool Response::readFrame(const std::vector<uint8_t>& data) {
            std::size_t offset = 0;
        ...
        std::vector<uint8_t> Response::writeFrame(const std::string& message, bool fin, Opcode opcode) {
        ...
        std::vector<uint8_t> Response::writeMaskedFrame(const std::string& message, bool fin, uint8_t opcode) {
            std::vector<uint8_t> frame;
